...
BRED = "\x1b[91m"; BGREEN = "\x1b[92m"; BYELLOW = "\x1b[93m"; BCYAN = "\x1b[96m"; BMAGENTA = "\x1b[95m"

def read_captured(name):
    path = os.path.join(CAPTURE_DIR, name)
    if not os.path.exists(path): return ""
    with open(path) as f: return f.read().rstrip()
...
"""
    ...
    fixes = [
        ...
        (f"{RED}worker-3{RESET} DiskPressure", f"{CYAN}kubectl{RESET} drain worker-3 --ignore-daemonsets"),
    ...
    rows = [
        ...
        ("Popeye score", f"{BRED}F{RESET}", f"{BGREEN}A{RESET}"),
